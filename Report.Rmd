...
All alnalysis in the following report were carried out using R 3.5.1 software.
...
1. The original lmBoot function only produced bootstrap saples for one covariate. This was change so that lmBoot_2 produces bootstrap estimates for a multiple number of covariates and calculates ocnfidence intervals for each parameter estimate. 

2. The use of the *lm* function was removed and the beta coefficients rather calculated using matrix clculations. 
...
Table ### illustrates differences in runtime for three different versions of the lmBoot function; the original, an improved version and a parallised version. Each function was timed on howlong it took to resaample 100, 1000, 10000 and 100000 samples.
