...
1. The use of the *lm* function was removed and the beta coefficients rather calculated using matrix clculations. 

$$
\beta = (X^TX)^{-1}X^TY
$$

2. *forloops* are known to be relatively slow and inefficient. Therefore, the *forloop* was replaced using *sapply* which applies a function to each element of a matrix. The function called *bootLM* was written to carry out the bootstrap algorithm. 
